...
The entrance of this project with enhanced support for specific JSON input.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="In-context learning script with enhanced JSON input support")
    ...
    parser.add_argument("--text_input", type=str, help="Text input path")
...
def main():
    ...
    # switch json input to text input
    # TODO
    if args.text_input:
        json_data = json.loads(args.text_input)
        cfg['task'] = json_data.get('task')
        instruction_data = json.loads(json_data.get('instruction', '{}'))
        cfg['instruction'] = instruction_data.get('instruction')
        schema = instruction_data.get('schema', [])
        cfg['instruction'] += f" Schema: {schema}"
        cfg['text_input'] = instruction_data.get('input')
