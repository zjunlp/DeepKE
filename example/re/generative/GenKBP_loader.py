...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file_src, file_tgt,neg_src ,neg_tgt ,batch_size, tokenizer, max_len, file_oracle=None, short_sampling_prob=0.1, sent_reverse_order=False, bi_uni_pipeline=[]):
        super().__init__()
        ...
        if file_oracle is None:
            ...
            with open(neg_src, "r", encoding='utf-8') as fneg_src, open(neg_tgt, "r", encoding='utf-8') as fneg_tgt:
                for neg_src1 , neg_tgt1 in zip(fneg_src, fneg_tgt):
                    neg_src_tk = tokenizer.tokenize(neg_src1.strip())
                    neg_tgt_tk = tokenizer.tokenize(neg_tgt1.strip())
                    assert len(neg_src_tk) > 0
                    assert len(neg_tgt_tk) > 0
                    #负样本
                    if neg_tgt1.strip() == '[True]':
                        self.ex_list.append((neg_src_tk, neg_tgt_tk , 0))
                    if neg_tgt1.strip() == '[False]':
                        self.ex_list.append((neg_src_tk, neg_tgt_tk,  1))
        else:
...
class Preprocess4Seq2seq(Pipeline):
    ...
    def __call__(self, instance):
        tokens_a, tokens_b, is_neg = instance[:3]
        ...
        if self.pos_shift:
            ...
            for pos in cand_pos:
                ...
                if (self.skipgram_prb > 0) and (self.skipgram_size >= 2) and (rand() < self.skipgram_prb):
                    ...
                    if self.mask_whole_word:
                        ...
                        st_pos, end_pos = pos, pos + 1  #进入的是这步，直接Mask
            ...
            if is_neg == -1:
                ...
                masked_weights = [10] * len(masked_tokens)
        ...
        return (input_ids, segment_ids, input_mask, mask_qkv, masked_ids, masked_pos, masked_weights, -1, self.task_idx, is_neg)
