...
def main():
    ...
    parser.add_argument("--neg_src_file", default=None, type=str,
                        help="The neg input data file name.")
    parser.add_argument("--neg_tgt_file", default=None, type=str,
                        help="The neg output data file name.")
    ...
    #Turn off at random initialization
    ...
    if args.do_train:
        ...
        neg_src = os.path.join(
            args.data_dir, args.neg_src_file if args.neg_src_file else 'neg_train.src')
        neg_tgt = os.path.join(
            args.data_dir, args.neg_tgt_file if args.neg_tgt_file else 'neg_tgt.src')

        train_dataset = seq2seq_loader.Seq2SeqDataset(
            fn_src, fn_tgt,neg_src,neg_tgt, args.train_batch_size, data_tokenizer, args.max_seq_length, file_oracle=file_oracle, bi_uni_pipeline=bi_uni_pipeline)
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            for step, batch in enumerate(iter_bar):
                ...
                if args.has_sentence_oracle:  #No
                    ...
                    input_ids, segment_ids, input_mask, mask_qkv, lm_label_ids, masked_pos, masked_weights, is_next, task_idx , is_neg = batch
                ...
                loss_tuple = model(input_ids, segment_ids, input_mask, lm_label_ids, is_next, masked_pos=masked_pos, masked_weights=masked_weights, task_idx=task_idx, masked_pos_2=oracle_pos, masked_weights_2=oracle_weights,
                                   masked_labels_2=oracle_labels, mask_qkv=mask_qkv, is_neg=is_neg )
            ...
            if (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                logger.info(
                    "** ** * Saving fine-tuned model and optimizer,model.{0}.bin ** ** * ".format(i_epoch))
